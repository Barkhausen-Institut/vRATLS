...
Module SignatureProt
  ...
  (** 
  Note: In the following Lemma, we use the simplified axiom Signature_correct_fail 
  because the Parameter [Signature_correct] in Signature.v file requires the access 
  to the seed used in KeyGen. In this proof context, we do not have access to the
  seed value used to produce (pk, sk), nor can we directly reason about the 
  internal `Run` computation.
  This axiom allows us to complete the indistinguishability proof and
  it implicitly assumes that
  pk and sk were generated by a consistent call to KeyGen.
  This is a limitation of the SSProve framework.
  *)
